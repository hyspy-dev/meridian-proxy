...
import meridian.protocol.AngledDamage;
import meridian.protocol.DamageEffects;
import meridian.protocol.EntityStatOnHit;
...
import meridian.protocol.TargetedDamage;
...
extends Interaction {
    ...
    public int serialize(@Nonnull ByteBuf buf) {
        ...
        if (this.targetedDamage != null) {
            ...
            for (var entry : this.targetedDamage.entrySet()) {
    ...
    public DamageEntityInteraction clone() {
        HashMap<String, TargetedDamage> m;
        ...
        if (this.settings != null) {
            HashMap<GameMode, InteractionSettings> m2 = new HashMap<GameMode, InteractionSettings>();
            for (var entry : this.settings.entrySet()) {
                m2.put(entry.getKey(), entry.getValue().clone());
            }
            copy.settings = m2;
        }
        ...
        AngledDamage[] angledDamageArray = copy.angledDamage = this.angledDamage != null ? (AngledDamage[])Arrays.stream(this.angledDamage).map(e -> e.clone()).toArray(AngledDamage[]::new) : null;
        if (this.targetedDamage != null) {
            HashMap<String, TargetedDamage> m3 = new HashMap<String, TargetedDamage>();
            for (var entry : this.targetedDamage.entrySet()) {
                m3.put(entry.getKey(), entry.getValue().clone());
            }
            copy.targetedDamage = m3;
        }
        copy.entityStatsOnHit = this.entityStatsOnHit != null ? (EntityStatOnHit[])Arrays.stream(this.entityStatsOnHit).map(e -> e.clone()).toArray(EntityStatOnHit[]::new) : null;
        return copy;
    }
