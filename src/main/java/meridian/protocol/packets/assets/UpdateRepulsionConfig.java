...
ToClientPacket {
    ...
    public static UpdateRepulsionConfig deserialize(@Nonnull ByteBuf buf, int offset) {
        ...
        if ((nullBits & 1) != 0) {
            ...
            obj.repulsionConfigs = new HashMap<Integer, RepulsionConfig>(repulsionConfigsCount);
            for (int i = 0; i < repulsionConfigsCount; ++i) {
                int key = buf.getIntLE(pos);
                RepulsionConfig val = RepulsionConfig.deserialize(buf, pos += 4);
                pos += RepulsionConfig.computeBytesConsumed(buf, pos);
                if (obj.repulsionConfigs.put(key, val) == null) continue;
    ...
    public UpdateRepulsionConfig clone() {
        ...
        if (this.repulsionConfigs != null) {
            HashMap<Integer, RepulsionConfig> m = new HashMap<Integer, RepulsionConfig>();
            for (var e : this.repulsionConfigs.entrySet()) {
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof UpdateRepulsionConfig)) {
            return false;
        }
        UpdateRepulsionConfig other = (UpdateRepulsionConfig)obj;
        return Objects.equals((Object)this.type, (Object)other.type) && this.maxId == other.maxId && Objects.equals(this.repulsionConfigs, other.repulsionConfigs);
    }
