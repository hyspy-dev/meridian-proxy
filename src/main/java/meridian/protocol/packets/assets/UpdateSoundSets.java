...
ToClientPacket {
    ...
    public static UpdateSoundSets deserialize(@Nonnull ByteBuf buf, int offset) {
        ...
        if ((nullBits & 1) != 0) {
            ...
            obj.soundSets = new HashMap<Integer, SoundSet>(soundSetsCount);
            for (int i = 0; i < soundSetsCount; ++i) {
                int key = buf.getIntLE(pos);
                SoundSet val = SoundSet.deserialize(buf, pos += 4);
                pos += SoundSet.computeBytesConsumed(buf, pos);
                if (obj.soundSets.put(key, val) == null) continue;
    ...
    public int computeSize() {
        ...
        if (this.soundSets != null) {
            int soundSetsSize = 0;
            for (var kvp : this.soundSets.entrySet()) {
    ...
    public UpdateSoundSets clone() {
        ...
        if (this.soundSets != null) {
            HashMap<Integer, SoundSet> m = new HashMap<Integer, SoundSet>();
            for (var e : this.soundSets.entrySet()) {
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof UpdateSoundSets)) {
            return false;
        }
        UpdateSoundSets other = (UpdateSoundSets)obj;
        return Objects.equals((Object)this.type, (Object)other.type) && this.maxId == other.maxId && Objects.equals(this.soundSets, other.soundSets);
    }
