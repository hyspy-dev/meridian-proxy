...
ToClientPacket {
    ...
    public static UpdateModelvfxs deserialize(@Nonnull ByteBuf buf, int offset) {
        ...
        if ((nullBits & 1) != 0) {
            ...
            obj.modelVFXs = new HashMap<Integer, ModelVFX>(modelVFXsCount);
            for (int i = 0; i < modelVFXsCount; ++i) {
                int key = buf.getIntLE(pos);
                ModelVFX val = ModelVFX.deserialize(buf, pos += 4);
                pos += ModelVFX.computeBytesConsumed(buf, pos);
                if (obj.modelVFXs.put(key, val) == null) continue;
    ...
    public int computeSize() {
        ...
        if (this.modelVFXs != null) {
            int modelVFXsSize = 0;
            for (var kvp : this.modelVFXs.entrySet()) {
    ...
    public UpdateModelvfxs clone() {
        ...
        if (this.modelVFXs != null) {
            HashMap<Integer, ModelVFX> m = new HashMap<Integer, ModelVFX>();
            for (var e : this.modelVFXs.entrySet()) {
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof UpdateModelvfxs)) {
            return false;
        }
        UpdateModelvfxs other = (UpdateModelvfxs)obj;
        return Objects.equals((Object)this.type, (Object)other.type) && this.maxId == other.maxId && Objects.equals(this.modelVFXs, other.modelVFXs);
    }
