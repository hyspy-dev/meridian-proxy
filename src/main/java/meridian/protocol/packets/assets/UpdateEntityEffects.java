...
ToClientPacket {
    ...
    public static UpdateEntityEffects deserialize(@Nonnull ByteBuf buf, int offset) {
        ...
        if ((nullBits & 1) != 0) {
            ...
            obj.entityEffects = new HashMap<Integer, EntityEffect>(entityEffectsCount);
            for (int i = 0; i < entityEffectsCount; ++i) {
                int key = buf.getIntLE(pos);
                EntityEffect val = EntityEffect.deserialize(buf, pos += 4);
                pos += EntityEffect.computeBytesConsumed(buf, pos);
                if (obj.entityEffects.put(key, val) == null) continue;
    ...
    public int computeSize() {
        ...
        if (this.entityEffects != null) {
            ...
            for (var kvp : this.entityEffects.entrySet()) {
    ...
    public UpdateEntityEffects clone() {
        ...
        if (this.entityEffects != null) {
            HashMap<Integer, EntityEffect> m = new HashMap<Integer, EntityEffect>();
            for (var e : this.entityEffects.entrySet()) {
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof UpdateEntityEffects)) {
            return false;
        }
        UpdateEntityEffects other = (UpdateEntityEffects)obj;
        return Objects.equals((Object)this.type, (Object)other.type) && this.maxId == other.maxId && Objects.equals(this.entityEffects, other.entityEffects);
    }
