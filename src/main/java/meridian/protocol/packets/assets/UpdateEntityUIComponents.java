...
ToClientPacket {
    ...
    public static UpdateEntityUIComponents deserialize(@Nonnull ByteBuf buf, int offset) {
        ...
        if ((nullBits & 1) != 0) {
            ...
            obj.components = new HashMap<Integer, EntityUIComponent>(componentsCount);
            for (int i = 0; i < componentsCount; ++i) {
                int key = buf.getIntLE(pos);
                EntityUIComponent val = EntityUIComponent.deserialize(buf, pos += 4);
                pos += EntityUIComponent.computeBytesConsumed(buf, pos);
                if (obj.components.put(key, val) == null) continue;
    ...
    public int computeSize() {
        ...
        if (this.components != null) {
            ...
            for (var kvp : this.components.entrySet()) {
    ...
    public UpdateEntityUIComponents clone() {
        ...
        if (this.components != null) {
            HashMap<Integer, EntityUIComponent> m = new HashMap<Integer, EntityUIComponent>();
            for (var e : this.components.entrySet()) {
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof UpdateEntityUIComponents)) {
            return false;
        }
        UpdateEntityUIComponents other = (UpdateEntityUIComponents)obj;
        return Objects.equals((Object)this.type, (Object)other.type) && this.maxId == other.maxId && Objects.equals(this.components, other.components);
    }
