...
ToClientPacket {
    ...
    public static UpdateCameraShake deserialize(@Nonnull ByteBuf buf, int offset) {
        ...
        if ((nullBits & 1) != 0) {
            ...
            obj.profiles = new HashMap<Integer, CameraShake>(profilesCount);
            for (int i = 0; i < profilesCount; ++i) {
                int key = buf.getIntLE(pos);
                CameraShake val = CameraShake.deserialize(buf, pos += 4);
    ...
    public UpdateCameraShake clone() {
        ...
        copy.type = this.type;
        if (this.profiles != null) {
            HashMap<Integer, CameraShake> m = new HashMap<Integer, CameraShake>();
            for (var e : this.profiles.entrySet()) {
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof UpdateCameraShake)) {
            return false;
        }
