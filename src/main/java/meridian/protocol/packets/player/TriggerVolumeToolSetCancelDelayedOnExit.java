...
public class TriggerVolumeToolSetCancelDelayedOnExit implements Packet, ToServerPacket {
   ...
   @Nonnull
   public static TriggerVolumeToolSetCancelDelayedOnExit deserialize(@Nonnull ByteBuf buf, int offset) {
      if (buf.readableBytes() - offset < 1) {
         throw ProtocolException.bufferTooSmall("TriggerVolumeToolSetCancelDelayedOnExit", 1, buf.readableBytes() - offset);
      }

      TriggerVolumeToolSetCancelDelayedOnExit obj = new TriggerVolumeToolSetCancelDelayedOnExit();
      obj.cancelDelayedOnExit = buf.getByte(offset + 0) != 0;
      int pos = offset + 1;
   ...
   public static TriggerVolumeToolSetCancelDelayedOnExit toObject(MemorySegment mem, int offset) {
      if (offset + 1 > mem.byteSize()) {
         throw ProtocolException.bufferTooSmall("TriggerVolumeToolSetCancelDelayedOnExit", offset + 1, (int)mem.byteSize());
      } else {
         return new TriggerVolumeToolSetCancelDelayedOnExit(
            PacketIO.readVarString("VolumeId", mem, offset + 1, 4096000, PacketIO.UTF8), mem.get(PacketIO.PROTO_BOOL, offset + 0)
         );
      }
   }
   ...
   public boolean equals(Object obj) {
      if (this == obj) {
         ...
         return !(obj instanceof TriggerVolumeToolSetCancelDelayedOnExit other)
            ? false
            : Objects.equals(this.volumeId, other.volumeId) && this.cancelDelayedOnExit == other.cancelDelayedOnExit;
      }
   }

   @Override
   public int hashCode() {
      return Objects.hash(this.volumeId, this.cancelDelayedOnExit);
   }
}
