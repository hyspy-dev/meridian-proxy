...
public class BuilderToolSetTransformationModeState implements Packet, ToServerPacket {
   ...
   public static BuilderToolSetTransformationModeState deserialize(@Nonnull ByteBuf buf, int offset) {
      if (buf.readableBytes() - offset < 1) {
         throw ProtocolException.bufferTooSmall("BuilderToolSetTransformationModeState", 1, buf.readableBytes() - offset);
      }
   ...
   public static BuilderToolSetTransformationModeState toObject(MemorySegment mem, int offset) {
      if (offset + 1 > mem.byteSize()) {
         throw ProtocolException.bufferTooSmall("BuilderToolSetTransformationModeState", offset + 1, (int)mem.byteSize());
      } else {
         return new BuilderToolSetTransformationModeState(mem.get(PacketIO.PROTO_BOOL, offset + 0));
      }
   }
