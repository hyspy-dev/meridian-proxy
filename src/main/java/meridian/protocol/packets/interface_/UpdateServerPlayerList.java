...
import meridian.protocol.packets.interface_.ServerPlayerListUpdate;
...
ToClientPacket {
    ...
    public static ValidationResult validateStructure(@Nonnull ByteBuf buffer, int offset) {
        ...
        if ((nullBits & 1) != 0) {
            ...
            if ((pos += playersCount * 32) > buffer.writerIndex()) {
    ...
    public UpdateServerPlayerList clone() {
        ...
        copy.players = this.players != null ? (ServerPlayerListUpdate[])Arrays.stream(this.players).map(e -> e.clone()).toArray(ServerPlayerListUpdate[]::new) : null;
        return copy;
    }
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof UpdateServerPlayerList)) {
            return false;
        }
