...
import meridian.protocol.ItemGridInfoDisplayMode;
import meridian.protocol.SubCategoryDefinition;
...
public class ItemCategory {
    ...
    public static ItemCategory deserialize(@Nonnull ByteBuf buf, int offset) {
        ...
        if ((nullBits & 8) != 0) {
            ...
            for (i = 0; i < childrenCount; ++i) {
                obj.children[i] = ItemCategory.deserialize(buf, elemPos);
                elemPos += ItemCategory.computeBytesConsumed(buf, elemPos);
            }
        }
        if ((nullBits & 0x10) != 0) {
            ...
            if ((long)(varPos4 + varIntLen) + (long)subCategoriesCount * 5L > (long)buf.readableBytes()) {
    ...
    public static int computeBytesConsumed(@Nonnull ByteBuf buf, int offset) {
        ...
        if ((nullBits & 8) != 0) {
            ...
            for (i = 0; i < arrLen; ++i) {
                pos3 += ItemCategory.computeBytesConsumed(buf, pos3);
            }
    ...
    public void serialize(@Nonnull ByteBuf buf) {
        ...
        if (this.subCategories != null) {
            ...
            for (SubCategoryDefinition subCategoryDefinition : this.subCategories) {
                subCategoryDefinition.serialize(buf);
            }
        } else {
    ...
    public int computeSize() {
        ...
        if (this.children != null) {
            int childrenSize = 0;
            for (ItemCategory itemCategory : this.children) {
                childrenSize += itemCategory.computeSize();
            }
        ...
        if (this.subCategories != null) {
            ...
            for (SubCategoryDefinition subCategoryDefinition : this.subCategories) {
                subCategoriesSize += subCategoryDefinition.computeSize();
            }
    ...
    public static ValidationResult validateStructure(@Nonnull ByteBuf buffer, int offset) {
        ...
        if ((nullBits & 8) != 0) {
            ...
            pos += VarInt.length(buffer, pos);
            for (i = 0; i < childrenCount; ++i) {
                structResult = ItemCategory.validateStructure(buffer, pos);
        ...
        if ((nullBits & 0x10) != 0) {
            ...
            pos = offset + 26 + subCategoriesOffset;
            if (pos >= buffer.writerIndex()) {
            ...
            pos += VarInt.length(buffer, pos);
            for (i = 0; i < subCategoriesCount; ++i) {
                structResult = SubCategoryDefinition.validateStructure(buffer, pos);
                if (!structResult.isValid()) {
    ...
    public ItemCategory clone() {
        ...
        copy.children = this.children != null ? (ItemCategory[])Arrays.stream(this.children).map(e -> e.clone()).toArray(ItemCategory[]::new) : null;
        copy.subCategories = this.subCategories != null ? (SubCategoryDefinition[])Arrays.stream(this.subCategories).map(e -> e.clone()).toArray(SubCategoryDefinition[]::new) : null;
        return copy;
    }
    ...
    public boolean equals(Object obj) {
        ...
        return Objects.equals(this.id, other.id) && Objects.equals(this.name, other.name) && Objects.equals(this.icon, other.icon) && this.order == other.order && Objects.equals((Object)this.infoDisplayMode, (Object)other.infoDisplayMode) && Arrays.equals(this.children, other.children) && Arrays.equals(this.subCategories, other.subCategories);
    }
    ...
    public int hashCode() {
        ...
        result = 31 * result + Objects.hashCode((Object)this.infoDisplayMode);
        result = 31 * result + Arrays.hashCode(this.children);
        result = 31 * result + Arrays.hashCode(this.subCategories);
        return result;
    }
}
