...
public class MemoriesConditionInteraction extends Interaction {
   ...
   public MemoriesConditionInteraction(
      ...
      @Nullable Map<Integer, Integer> memoriesNext,
      int failed
   ) {
   ...
   public static MemoriesConditionInteraction deserialize(@Nonnull ByteBuf buf, int offset) {
      if (buf.readableBytes() - offset < 39) {
         throw ProtocolException.bufferTooSmall("MemoriesConditionInteraction", 39, buf.readableBytes() - offset);
      }
      ...
      if ((nullBits & 32) != 0) {
         ...
         if (varPosBase5 < 0 || varPosBase5 > buf.writerIndex() - offset - 39) {
            throw ProtocolException.invalidOffset("MemoriesNext", varPosBase5, buf.readableBytes());
         }

         int varPos5 = offset + 39 + varPosBase5;
         int memoriesNextCount = VarInt.peek(buf, varPos5);
         if (memoriesNextCount < 0) {
            throw ProtocolException.invalidVarInt("MemoriesNext");
         }

         int varIntLen = VarInt.size(memoriesNextCount);
   ...
   public static int computeBytesConsumed(@Nonnull ByteBuf buf, int offset) {
      ...
      if ((nullBits & 32) != 0) {
         ...
         if (fieldOffset5 < 0 || fieldOffset5 > buf.writerIndex() - offset - 39) {
            throw ProtocolException.invalidOffset("MemoriesNext", fieldOffset5, maxEnd);
         }
   ...
   public static InteractionEffects getEffects(MemorySegment mem, int offset) {
      return hasEffects(mem, offset) ? InteractionEffects.toObject(mem, offset + getValidatedOffset(mem, offset, 15, 39, "Effects")) : null;
   }
   ...
   public static Map<GameMode, InteractionSettings> getSettings(MemorySegment mem, int offset) {
      ...
      int off = offset + getValidatedOffset(mem, offset, 19, 39, "Settings");
   ...
   public static InteractionRules getRules(MemorySegment mem, int offset) {
      return hasRules(mem, offset) ? InteractionRules.toObject(mem, offset + getValidatedOffset(mem, offset, 23, 39, "Rules")) : null;
   }
   ...
   public static int[] getTags(MemorySegment mem, int offset) {
      ...
      int off = offset + getValidatedOffset(mem, offset, 27, 39, "Tags");
      long packed = VarInt.getWithLength(mem, off);
      int len = (int)packed;
      if (len < 0) {
   ...
   public static InteractionCameraSettings getCamera(MemorySegment mem, int offset) {
      return hasCamera(mem, offset) ? InteractionCameraSettings.toObject(mem, offset + getValidatedOffset(mem, offset, 31, 39, "Camera")) : null;
   }

   @Nullable
   public static Map<Integer, Integer> getMemoriesNext(MemorySegment mem) {
      return getMemoriesNext(mem, 0);
   }

   @Nullable
   public static Map<Integer, Integer> getMemoriesNext(MemorySegment mem, int offset) {
      if (!hasMemoriesNext(mem, offset)) {
         return null;
      }

      int off = offset + getValidatedOffset(mem, offset, 35, 39, "MemoriesNext");
      long packed = VarInt.getWithLength(mem, off);
      int len = (int)packed;
      if (len < 0) {
         throw ProtocolException.negativeLength("MemoriesNext", len);
      }

      if (len > 4096000) {
         throw ProtocolException.dictionaryTooLarge("MemoriesNext", len, 4096000);
      }
   ...
   public static boolean hasMemoriesNext(MemorySegment mem, int offset) {
      byte b = mem.get(PacketIO.PROTO_BYTE, offset + 0);
      return (b & 32) != 0;
   }
   ...
   public static MemoriesConditionInteraction toObject(MemorySegment mem, int offset) {
      if (offset + 39 > mem.byteSize()) {
         throw ProtocolException.bufferTooSmall("MemoriesConditionInteraction", offset + 39, (int)mem.byteSize());
      }
      ...
      Map<Integer, Integer> memoriesNext = null;
      if (hasMemoriesNext(mem, offset)) {
         int off = offset + getValidatedOffset(mem, offset, 35, 39, "MemoriesNext");
         long packed = VarInt.getWithLength(mem, off);
         int len = (int)packed;
         ...
         for (int i = 0; i < len; i++) {
            ...
            int value = mem.get(PacketIO.PROTO_INT, off);
            off += 4;
            if (memoriesNext.put(key, value) != null) {
               throw ProtocolException.duplicateKey("MemoriesNext", key);
            }
         }
      }

      return new MemoriesConditionInteraction(
         WaitForDataFrom.fromValue(mem.get(PacketIO.PROTO_BYTE, offset + 1)),
         hasEffects(mem, offset) ? InteractionEffects.toObject(mem, offset + getValidatedOffset(mem, offset, 15, 39, "Effects")) : null,
         mem.get(PacketIO.PROTO_FLOAT, offset + 2),
         mem.get(PacketIO.PROTO_FLOAT, offset + 6),
         mem.get(PacketIO.PROTO_BOOL, offset + 10),
         settings,
         hasRules(mem, offset) ? InteractionRules.toObject(mem, offset + getValidatedOffset(mem, offset, 23, 39, "Rules")) : null,
         tags,
         hasCamera(mem, offset) ? InteractionCameraSettings.toObject(mem, offset + getValidatedOffset(mem, offset, 31, 39, "Camera")) : null,
         memoriesNext,
   ...
   public int serialize(@Nonnull ByteBuf buf) {
      ...
      if (this.memoriesNext != null) {
         ...
         for (Entry<Integer, Integer> e : this.memoriesNext.entrySet()) {
   ...
   public static ValidationResult validateStructure(@Nonnull ByteBuf buffer, int offset) {
      ...
      if ((nullBits & 32) != 0) {
         ...
         pos += VarInt.size(memoriesNextCount);
   ...
   public boolean equals(Object obj) {
      if (this == obj) {
         ...
         return !(obj instanceof MemoriesConditionInteraction other)
            ? false
            : Objects.equals(this.waitForDataFrom, other.waitForDataFrom)
               ...
               && Objects.equals(this.memoriesNext, other.memoriesNext)
               && this.failed == other.failed;
      }
   }
   ...
   public int hashCode() {
      ...
      return 31 * result + Integer.hashCode(this.failed);
   }
}
