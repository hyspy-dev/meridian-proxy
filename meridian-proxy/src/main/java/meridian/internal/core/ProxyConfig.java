...
public record ProxyConfig(
    ...
    /** True when launched as the launcher's server jar ({@code --backup-dir} present). */
    public boolean launchedByLauncher() {
        return findArg(originalArgs, "--backup-dir") != null;
    }

    /** Directory containing the running proxy jar; falls back to CWD when unknown. */
    public static Path jarDir() {
        try {
            File jar = new File(ProxyConfig.class.getProtectionDomain()
                    .getCodeSource().getLocation().toURI());
            File parent = jar.getParentFile();
            if (parent != null) return parent.toPath();
        } catch (Exception ignored) {}
        return Paths.get(".").toAbsolutePath().normalize();
    }

    /** Filesystem-safe {@code host_port} token for namespacing per-server folders. */
    public static String sanitizeHostPort(String host, int port) {
        return (host + "_" + port).replaceAll("[^a-zA-Z0-9._-]", "_");
    }
