...
import meridian.internal.core.PacketCodec;
import meridian.internal.core.PacketForwarder;
import meridian.internal.core.PacketRouter;
import meridian.internal.core.ProxySessionImpl;
import meridian.internal.core.QuicConfig;
import meridian.internal.handler.ConnectObserver;
import meridian.internal.handler.BackAuthHandler;
import meridian.internal.handler.FrontAuthHandler;
import meridian.api.packet.PacketHandler;
import meridian.internal.handler.PhaseTracker;
import meridian.internal.handler.RouteGuard;
import meridian.internal.handler.ServerAccessLogger;
...
public class ProxyFrontendHandler extends ChannelInboundHandlerAdapter {
    ...
    private void connectBackend(ChannelHandlerContext ctx, QuicChannel inboundChannel,
        ...
        try {
            ...
            b.group(inboundChannel.eventLoop())
                    ...
                    // Large UDP socket buffers — see the matching note in ProxyServer.
                    // This is the backend (server-facing) leg, which receives the
                    // multi-megabyte setup payload from the real server.
    ...
    private void linkStreams(QuicStreamChannel clientStream, QuicStreamChannel serverStream) {
        ...
        ProxySessionImpl session = new ProxySessionImpl(auth);
    ...
    private void setupStreamPipeline(QuicStreamChannel stream, Direction dir,
            QuicStreamChannel target, ProxySessionImpl session) {
        ...
        for (PacketHandlerFactory factory : handlerRegistry.factoriesFor(dir)) {
            try {
                handlers.add(factory.create(dir, session));
            } catch (Exception e) {
        ...
        stream.pipeline().addLast(new PacketRouter(forwarder, session, dir, handlers, eventBus));
