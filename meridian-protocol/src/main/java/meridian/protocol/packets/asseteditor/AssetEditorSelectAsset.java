...
public class AssetEditorSelectAsset implements Packet, ToServerPacket {
   ...
   public static AssetEditorSelectAsset deserialize(@Nonnull ByteBuf buf, int offset) {
      if (buf.readableBytes() - offset < 1) {
         throw ProtocolException.bufferTooSmall("AssetEditorSelectAsset", 1, buf.readableBytes() - offset);
      }
   ...
   public static AssetEditorSelectAsset toObject(MemorySegment mem, int offset) {
      if (offset + 1 > mem.byteSize()) {
         throw ProtocolException.bufferTooSmall("AssetEditorSelectAsset", offset + 1, (int)mem.byteSize());
      } else {
         return new AssetEditorSelectAsset(hasPath(mem, offset) ? AssetPath.toObject(mem, offset + 1) : null);
      }
   }
