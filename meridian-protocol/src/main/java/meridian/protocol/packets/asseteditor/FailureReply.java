...
public class FailureReply implements Packet, ToServerPacket, ToClientPacket {
   ...
   public static FailureReply toObject(MemorySegment mem, int offset) {
      if (offset + 5 > mem.byteSize()) {
         throw ProtocolException.bufferTooSmall("FailureReply", offset + 5, (int)mem.byteSize());
      } else {
         return new FailureReply(mem.get(PacketIO.PROTO_INT, offset + 1), hasMessage(mem, offset) ? FormattedMessage.toObject(mem, offset + 5) : null);
      }
   }
