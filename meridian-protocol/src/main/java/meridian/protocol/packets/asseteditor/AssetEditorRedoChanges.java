...
public class AssetEditorRedoChanges implements Packet, ToServerPacket {
   ...
   public static AssetEditorRedoChanges deserialize(@Nonnull ByteBuf buf, int offset) {
      if (buf.readableBytes() - offset < 5) {
         throw ProtocolException.bufferTooSmall("AssetEditorRedoChanges", 5, buf.readableBytes() - offset);
      }
   ...
   public static AssetEditorRedoChanges toObject(MemorySegment mem, int offset) {
      if (offset + 5 > mem.byteSize()) {
         throw ProtocolException.bufferTooSmall("AssetEditorRedoChanges", offset + 5, (int)mem.byteSize());
      } else {
         return new AssetEditorRedoChanges(mem.get(PacketIO.PROTO_INT, offset + 1), hasPath(mem, offset) ? AssetPath.toObject(mem, offset + 5) : null);
      }
   }
   ...
   public boolean equals(Object obj) {
      if (this == obj) {
         ...
         return !(obj instanceof AssetEditorRedoChanges other) ? false : this.token == other.token && Objects.equals(this.path, other.path);
      }
   }
