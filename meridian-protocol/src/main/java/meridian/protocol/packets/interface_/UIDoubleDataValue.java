...
public class UIDoubleDataValue extends UIDataValue {
   ...
   @Nonnull
   public static UIDoubleDataValue deserialize(@Nonnull ByteBuf buf, int offset) {
      if (buf.readableBytes() - offset < 8) {
         throw ProtocolException.bufferTooSmall("UIDoubleDataValue", 8, buf.readableBytes() - offset);
      }

      UIDoubleDataValue obj = new UIDoubleDataValue();
   ...
   public static UIDoubleDataValue toObject(MemorySegment mem, int offset) {
      if (offset + 8 > mem.byteSize()) {
         throw ProtocolException.bufferTooSmall("UIDoubleDataValue", offset + 8, (int)mem.byteSize());
      } else {
         return new UIDoubleDataValue(mem.get(PacketIO.PROTO_DOUBLE, offset + 0));
      }
   }
   ...
   public boolean equals(Object obj) {
      if (this == obj) {
         ...
         return obj instanceof UIDoubleDataValue other ? this.value == other.value : false;
      }
   }
