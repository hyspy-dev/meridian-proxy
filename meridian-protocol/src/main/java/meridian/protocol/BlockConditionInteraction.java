...
public class BlockConditionInteraction extends SimpleBlockInteraction {
   ...
   public static BlockConditionInteraction deserialize(@Nonnull ByteBuf buf, int offset) {
      if (buf.readableBytes() - offset < 44) {
         throw ProtocolException.bufferTooSmall("BlockConditionInteraction", 44, buf.readableBytes() - offset);
      }
      ...
      if ((nullBits & 32) != 0) {
         ...
         if (varPosBase5 < 0 || varPosBase5 > buf.writerIndex() - offset - 44) {
            throw ProtocolException.invalidOffset("Matchers", varPosBase5, buf.readableBytes());
         }
         ...
         int varIntLen = VarInt.size(matchersCount);
   ...
   public static int computeBytesConsumed(@Nonnull ByteBuf buf, int offset) {
      ...
      if ((nullBits & 32) != 0) {
         ...
         if (fieldOffset5 < 0 || fieldOffset5 > buf.writerIndex() - offset - 44) {
            throw ProtocolException.invalidOffset("Matchers", fieldOffset5, maxEnd);
         }
   ...
   public static BlockMatcher[] getMatchers(MemorySegment mem, int offset) {
      ...
      int off = offset + getValidatedOffset(mem, offset, 40, 44, "Matchers");
      long packed = VarInt.getWithLength(mem, off);
      int len = (int)packed;
      if (len < 0) {
      ...
      if (off + lenOffset + len > mem.byteSize()) {
         throw ProtocolException.bufferTooSmall("Matchers", off + lenOffset + len, (int)mem.byteSize());
      }

      off += lenOffset;
      BlockMatcher[] data = new BlockMatcher[len];

      for (int i = 0; i < len; i++) {
         data[i] = BlockMatcher.toObject(mem, off);
   ...
   public static BlockConditionInteraction toObject(MemorySegment mem, int offset) {
      if (offset + 44 > mem.byteSize()) {
         throw ProtocolException.bufferTooSmall("BlockConditionInteraction", offset + 44, (int)mem.byteSize());
      }
      ...
      if (hasMatchers(mem, offset)) {
         ...
         off += lenOffset;
         matchers = new BlockMatcher[len];

         for (int i = 0; i < len; i++) {
            matchers[i] = BlockMatcher.toObject(mem, off);
            off += matchers[i].computeSize();
         }
      }

      return new BlockConditionInteraction(
